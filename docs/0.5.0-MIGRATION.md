...
| `0.6.0` | `ERROR` | Old names still resolve at runtime, so already compiled callers keep working, but they no longer compile from source. Implementations of the interfaces must override the participial methods. | Migrate all call sites and implementations before upgrading. |
...
- **At `0.6.0`**, the deprecation level of the imperative methods becomes
  `ERROR`, and the delegation direction inside the interface is reversed:
  the participial methods become the abstract members that implementations
  must provide, while the deprecated imperative methods gain default
  ...
  participial methods — overrides of the deprecated names are no longer
  ...
  against `0.5.x` keeps working if it already overrides the participial methods
  as shown above. For any other implementation this is a binary-incompatible
  change on every target. On the JVM the first call of a participial method on
