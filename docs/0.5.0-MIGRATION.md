...
| `0.6.0` | `ERROR` | Old names still resolve at runtime, so already compiled callers keep working, but they no longer compile from source. Implementations of the interfaces must override the participial functions. | Migrate all call sites and implementations before upgrading. |
...
- **Existing implementations compiled against `0.4.x` continue to work at
  runtime against `0.5.x`.** The new participial functions are provided as
...
- **When you recompile against `0.5.x`**, move your implementation into the
  new participial functions and have each deprecated override delegate to it:
...
- **At `0.6.0`**, the deprecation level of the imperative functions becomes
  `ERROR`, and the delegation direction inside the interface is reversed:
  the participial functions become the abstract members that implementations
  must provide, while the deprecated imperative functions gain default
  implementations that delegate to the new participial ones.

  As a result, third-party implementations only need to override the
  participial functions — overrides of the deprecated names are no longer
  required and can simply be deleted. No `@Suppress` for the deprecation
  error is needed, because you no longer override an error-deprecated
  function.

  This step is binary compatible for callers only. An implementation compiled
  against `0.5.x` keeps working if it already overrides the participial functions
  as shown above. For any other implementation this is a binary-incompatible
  change on every target. On the JVM the first call of a participial function on
  such a class throws `AbstractMethodError`. On the klib-based targets the
  build succeeds silently and the same call throws `IrLinkageError` at runtime.
- **From `0.7.0`**, the imperative functions are removed from the interface and
  only the participial functions need to be implemented.
