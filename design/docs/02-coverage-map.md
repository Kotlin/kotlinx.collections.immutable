...
73 sample functions, 114 `@sample` tags. All references use the prefix `kotlinx.collections.immutable.samples`. Overload clusters (the `Iterable`/`Array`/`Sequence` variants of one operation) share one sample.
...
## Interface files (39 tags)

| File | Tagged declarations | Samples |
|---|---|---|
| `ImmutableCollection.kt` (10) | `ImmutableCollection`, `PersistentCollection` class docs; `adding`, `addingAll`, `removing`, `removingAll(elements)`, `removingAll(predicate)`, `retainingAll`; `builder()`; `Builder.build()` | `PersistentCollectionSamples.immutableCollection`, `.persistentCollection`, same-named functions for the participial members (`removingAll` pair maps to `removingAllElements`/`removingAllPredicate`), `.builder` (shared by both builder members) |
...
| `ImmutableSet.kt` (8) | `ImmutableSet`, `PersistentSet` class docs; `adding`, `addingAll`, `removing`, `removingAll(elements)`, `removingAll(predicate)`, `retainingAll` | same-named functions in `PersistentSetSamples` (`removingAll` pair maps to `removingAllElements`/`removingAllPredicate`) |
...
Untagged by design: deprecated declarations (hidden by `skipDeprecated`), `cleared()` (trivial), the `Builder` interface class docs, and the overrides that only narrow the return type — `builder()` and `Builder.build()` in `PersistentList`/`PersistentSet`, `keys`/`values`/`entries` in `ImmutableMap` — which have no own KDoc (a tag-only KDoc block would suppress the docs they inherit from `PersistentCollection` and `Map`).
...
Also new: class KDoc for `ImmutableCollectionAdapter`, `ImmutableListAdapter`, `ImmutableSetAdapter`, `ImmutableMapAdapter` (no-copy wrapping, full delegation, caller guarantees the underlying collection never changes). The validation pass added class KDoc for `AbstractPersistentList`, the one public class in the `implementations` packages (no sample; see [03-validation.md](03-validation.md)).
...
- `Iterable<T>.toImmutableSet()` promises iteration "in the same order as in this iterable", which is only vacuously true on the receiver-returned-as-is path when the receiver is an unordered `ImmutableSet` (the promised order is then the receiver's own unspecified order); the sample uses a `listOf` receiver where the promise is meaningful.
...
- `AbstractPersistentList` in `kotlinx.collections.immutable.implementations.immutableList` is public (ABI-frozen, exported in `module-info.java`) and shows up in the API reference. Whether to suppress the `implementations` packages from the docs is an open API-policy question.
