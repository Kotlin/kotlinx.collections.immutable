...
Part of the stabilization effort: give the core module's non-deprecated public API concise KDoc and runnable `@sample` snippets, following the conventions of the Kotlin standard library and kotlinx-datetime.

Result: 73 sample functions in `core/commonTest/src/samples/` referenced by 114 `@sample` tags — 75 in `extensions.kt` (every non-deprecated top-level function) and 39 across the four interface files (class-level docs, the participial members, the builder workflow). The four adapter classes in `kotlinx.collections.immutable.adapters`, previously undocumented, got class KDoc. Four factual gaps in existing docs were fixed and a stale commented-out prototype removed along the way (see [02-coverage-map.md](02-coverage-map.md)).

## Infrastructure

Samples live inside the existing `commonTest` source set (`core/commonTest/src/samples/`, package `kotlinx.collections.immutable.samples`), so they compile for every target and their `check(...)` assertions run as ordinary tests on JVM, JS, both Wasm targets, and the native targets runnable on the CI hosts — on every CI run. Dokka embeds the same functions into the rendered API reference, and the already-applied `kotlin-playground-samples-plugin` makes them editable and runnable in the browser.
...
- Referential `===` checks appear only where the docs promise "this instance" and the implementation verifiably short-circuits (the no-op paths of the ordered set and map samples, `toPersistentList` on a persistent receiver). Everything else uses `==`.
...
./gradlew :kotlinx-collections-immutable:jsNodeTest :kotlinx-collections-immutable:wasmJsNodeTest :kotlinx-collections-immutable:wasmWasiNodeTest
./gradlew :kotlinx-collections-immutable:dokkaGeneratePublicationHtml   # fails on any unresolved @sample
./gradlew :kotlinx-collections-immutable:checkLegacyAbi                 # docs and samples must not move the ABI
```

Native targets run the same tests on CI (`check` on each TeamCity host).
...
- [03-validation.md](03-validation.md): the validation pass — what was checked, what it found, what changed.
