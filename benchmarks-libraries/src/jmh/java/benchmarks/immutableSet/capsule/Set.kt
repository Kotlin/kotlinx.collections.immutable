...
open class Add {
    ...
    @Benchmark
    fun add(): io.usethesource.capsule.Set.Immutable<IntWrapper> {
...
open class Remove {
    ...
    @Benchmark
    fun remove(): io.usethesource.capsule.Set.Immutable<IntWrapper> {
        var set = persistentSet
        repeat(times = size) { index ->
            set = set.__remove(elements[index])
        }
        return set
    }
}
...
private fun persistentSetAdd(elements: List<IntWrapper>): io.usethesource.capsule.Set.Immutable<IntWrapper> {
