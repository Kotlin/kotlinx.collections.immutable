...
class PersistentHashMapTest {
    ...
    fun `putAll should not duplicate a key stored in a bottom-level collision node`() {
        val sum = persistentUnorderedMapOf(a1 to 1, a2 to 2) + persistentUnorderedMapOf(a1 to 10, sibling to 3)
        assertEquals(3, sum.size)
        assertEquals(persistentUnorderedMapOf(a1 to 10, a2 to 2, sibling to 3), sum)
        assertEquals(10, sum[a1])

        val reversedSum = persistentUnorderedMapOf(a1 to 10, sibling to 3) + persistentUnorderedMapOf(a1 to 1, a2 to 2)
        assertEquals(3, reversedSum.size)
        assertEquals(persistentUnorderedMapOf(a1 to 1, a2 to 2, sibling to 3), reversedSum)
        assertEquals(1, reversedSum[a1])
    }

    @Test
    fun `putAll should insert a new key into a bottom-level collision node`() {
        val expected = persistentUnorderedMapOf(a1 to 1, a2 to 2, a3 to 4, sibling to 3)

        val sum = persistentUnorderedMapOf(a1 to 1, a2 to 2) + persistentUnorderedMapOf(a3 to 4, sibling to 3)
        assertEquals(4, sum.size)
        assertEquals(expected, sum)

        val reversedSum = persistentUnorderedMapOf(a3 to 4, sibling to 3) + persistentUnorderedMapOf(a1 to 1, a2 to 2)
    ...
    fun `putAll should take the value of the argument for a key held by both collision nodes`() {
        val receiver = persistentUnorderedMapOf(a1 to 1, a2 to 2)
        val argument = persistentUnorderedMapOf(a1 to 10, a2 to 20)

        assertEquals(persistentUnorderedMapOf(a1 to 10, a2 to 20), receiver.puttingAll(argument))
        assertEquals(persistentUnorderedMapOf(a1 to 1, a2 to 2), argument.puttingAll(receiver))
    }

    @Test
    fun `putAll should merge partially overlapping collision nodes`() {
        val receiver = persistentUnorderedMapOf(a1 to 1, a2 to 2)
        val argument = persistentUnorderedMapOf(a2 to 20, a3 to 4)

        val sum = receiver.puttingAll(argument)
        assertEquals(3, sum.size)
        assertEquals(persistentUnorderedMapOf(a1 to 1, a2 to 20, a3 to 4), sum)

        val reversedSum = argument.puttingAll(receiver)
        assertEquals(3, reversedSum.size)
        assertEquals(persistentUnorderedMapOf(a1 to 1, a2 to 2, a3 to 4), reversedSum)
    }

    @Test
    fun `putAll should return a new map when the argument only replaces values of a collision node`() {
        val receiver = persistentUnorderedMapOf(a1 to 1, a2 to 2, a3 to 4, sibling to 3)
        val argument = persistentUnorderedMapOf(a1 to 10, a2 to 20)

        val sum = receiver.puttingAll(argument)
        assertNotSame(receiver, sum)
        assertEquals(4, sum.size)
        assertEquals(persistentUnorderedMapOf(a1 to 10, a2 to 20, a3 to 4, sibling to 3), sum)

        val reversedSum = argument.puttingAll(receiver)
        assertEquals(4, reversedSum.size)
        assertEquals(persistentUnorderedMapOf(a1 to 1, a2 to 2, a3 to 4, sibling to 3), reversedSum)
    }

    @Test
    fun `putAll should keep a null value the argument adds to a collision node`() {
        val receiver = persistentUnorderedMapOf(a1 to 1, a2 to null)
        val argument = persistentUnorderedMapOf(a2 to 20, a3 to null)

        val sum = receiver.puttingAll(argument)
        assertEquals(3, sum.size)
        assertEquals(persistentUnorderedMapOf(a1 to 1, a2 to 20, a3 to null), sum)
        assertNull(sum[a3])
    }
    ...
    fun `putAll should take a value of the argument that is equal but not the same instance`() {
        ...
        val receiver = persistentUnorderedMapOf(a1 to value, a2 to value)
        val argument = persistentUnorderedMapOf(a1 to equalValue, a2 to value)
    ...
    fun `putAll should return the same map when the argument brings no new values`() {
        val one = "one"
        val two = "two"
        val receiver = persistentUnorderedMapOf(a1 to one, a2 to two, sibling to "three")
        val argument = persistentUnorderedMapOf(a1 to one, a2 to two)
